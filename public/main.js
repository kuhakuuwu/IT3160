// import { gridInitialize, generator, grid } from '../test/maze.js';
// import { AStarAlgorithm } from '../test/astar.js';
...
function BFSAlgorithm() {
    ...
    if (!checkGrid()) return null;
    let queue = [];
    let visited = Array.from({length: grid.width}, () => Array(grid.height).fill(false));
    queue.push({x: source.x, y: source.y});
    visited[source.x][source.y] = true;
    gridDetail[source.x][source.y].parent = {x: source.x, y: source.y};
    considered = [];
    cellConsidered = 0;
    while (queue.length > 0) {
        let current = queue.shift();
        gridDetail[current.x][current.y].isConsidered = true;
        considered.push({x: current.x, y: current.y});
        cellConsidered++;
        if (isNode(current.x, current.y, destination)) {
            hasPath = true;
            return pathTracing();
        ...
        for (let i = 0; i < candidate.length; i++) {
            ...
            if (isInsideGrid(nx, ny) && !isWall(nx, ny) && !visited[nx][ny]) {
                visited[nx][ny] = true;
                gridDetail[nx][ny].parent = {x: current.x, y: current.y};
...
function GreedyAlgorithm() {
    ...
    let closed = [];
    gridDetail[source.x][source.y].parent = {x: source.x, y: source.y};
    let heuristicIndex = $('input[name="heuristic"]:checked').val();
    // Tính h cho source
    let hSource = gridDetail[source.x][source.y]['heuristic' + heuristicIndex](destination, source);
    open.push({x: source.x, y: source.y, h: hSource});
    considered = [];
    cellConsidered = 0;
    while (open.length > 0) {
        ...
        for (let i = 0; i < candidate.length; i++) {
            ...
            if (isInsideGrid(nx, ny) && !isWall(nx, ny) && !closed.some(n => n.x === nx && n.y === ny) && !open.some(n => n.x === nx && n.y === ny)) {
                gridDetail[nx][ny].parent = {x: current.x, y: current.y};
                // Tính h cho node mới
                let h = gridDetail[nx][ny]['heuristic' + heuristicIndex](destination, source);
                console.log('h:', h, 'heuristic:', heuristicIndex, 'node:', nx, ny);
                open.push({x: nx, y: ny, h: h});
            }
        }
    }
    return null;
...
function search() {
    let algo = $('input[name="algorithm"]:checked').val();
...
$('input[name="algorithm"]').on('change', function() {
    if ($('input[name="algorithm"]:checked').val() === 'bfs') {
        $('.heuristic-group').hide(); // Ẩn toàn bộ nhóm heuristic
        // Nếu muốn disable thay vì ẩn:
        // $('.heuristic-group input').prop('disabled', true);
    } else {
        $('.heuristic-group').show(); // Hiện lại khi không phải BFS
        // $('.heuristic-group input').prop('disabled', false);
    }
});
